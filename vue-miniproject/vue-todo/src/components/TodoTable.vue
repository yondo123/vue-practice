  ...
  <div>
    <section class="mainTodo">
      <table class="todo-list">
        ...
        <tbody>
          <tr
            v-for="item in this.todoItem"
            v-bind:key="item.todoItem"
            v-bind:class="{ active: item.removeFlag == false }"
          >
            <td>
              <div>{{ item.todo }}</div>
            </td>
            <td>
              <div>{{ formatKoreanTime(item.create) }}</div>
            </td>
            <td>
              <div>
                <span
                  ><i
                    v-bind:class="getTodoFlagIcon(item.complete)"
                    v-on:click="todoToggle(item.index)"
                  ></i
                ></span>
              </div>
            </td>
            <td v-on:click="removeItem(item.index)">
              <div><i class="fas fa-eraser todo-delete"></i></div>
            </td>
          </tr>
        </tbody>
      </table>
